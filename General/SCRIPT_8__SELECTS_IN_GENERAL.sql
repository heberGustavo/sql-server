...
/******* IN / NOT IN *******/
SELECT * FROM TB_EDITORA WHERE EDT_ID IN (1,4)
SELECT * FROM TB_EDITORA WHERE EDT_ID NOT IN (1,4)


/******* Calculating columns *******/
--CREATE TABLE TB_VENDAS (
--	VND_ID SMALLINT PRIMARY KEY IDENTITY,
--	VND_PRECO_PRODUTO MONEY NOT NULL,
--	VND_QUANT_ITENS TINYINT NOT NULL,
--	VND_PRECO_TOTAL AS VND_PRECO_PRODUTO * VND_QUANT_ITENS PERSISTED
--);
--GO

--INSERT INTO TB_VENDAS (VND_PRECO_PRODUTO, VND_QUANT_ITENS) VALUES (100, 1);
--INSERT INTO TB_VENDAS (VND_PRECO_PRODUTO, VND_QUANT_ITENS) VALUES (100, 2);
--INSERT INTO TB_VENDAS (VND_PRECO_PRODUTO, VND_QUANT_ITENS) VALUES (100, 3);
--INSERT INTO TB_VENDAS (VND_PRECO_PRODUTO, VND_QUANT_ITENS) VALUES (100, 4);
--INSERT INTO TB_VENDAS (VND_PRECO_PRODUTO, VND_QUANT_ITENS) VALUES (100, 5);

SELECT SUM(VND_PRECO_TOTAL) FROM  TB_VENDAS


/*STRINGS*/
SELECT 'Eu gosto do livro: ' + L.LVR_NOME FROM TB_LIVRO L
...
/* SUB QUERIES */
SELECT RESULTADO.EDT_NOME, SUM(RESULTADO.QUANT) AS QUANT_LIVROS FROM (
	SELECT E.EDT_NOME, L.LVR_NOME, COUNT(1) QUANT
	FROM TB_LIVRO L 
	INNER JOIN TB_EDITORA E ON E.EDT_ID = L.EDT_ID
	GROUP BY E.EDT_NOME, L.LVR_NOME
) AS RESULTADO
GROUP BY RESULTADO.EDT_NOME
ORDER BY QUANT_LIVROS


/* CTE - Common Table Expression */
WITH ConsultaCTE (EDT_NOME, QUANT) AS
(
	SELECT E.EDT_NOME, COUNT(1) QUANT
	FROM TB_LIVRO L 
	INNER JOIN TB_EDITORA E ON E.EDT_ID = L.EDT_ID
	GROUP BY E.EDT_NOME, L.LVR_NOME
)

SELECT EDT_NOME, SUM(QUANT) AS QUANT_LIVROS
FROM ConsultaCTE
GROUP BY EDT_NOME
ORDER BY QUANT_LIVROS